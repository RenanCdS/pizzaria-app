...
export async function updateProductByCode({ code, description, price}) {
    return new Promise((resolve, reject) => {
        const query = `UPDATE TB_PRODUCT SET DESCRIPTION = ?, PRICE = ?
                    WHERE PRODUCT_ID = ?; `;
        const connection = getDbConnection();

        connection.transaction(transaction => {
            transaction.executeSql(query, [description, price, code],
                (_, result) => {
                    resolve(result.rowsAffected > 0);
                });
        }, error => {
